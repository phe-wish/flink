...
/**
  * Validator for {@link Msgpack}.
  */
@Internal
public class MsgpackValidator extends FormatDescriptorValidator {

	public static final String FORMAT_TYPE_VALUE = "msgpack";
	public static final String FORMAT_SCHEMA = "format.schema";
	public static final String FORMAT_MSGPACK_SCHEMA = "format.msgpack-schema";
	...
	public void validate(DescriptorProperties properties) {
		...
		final boolean hasSchemaString = properties.containsKey(FORMAT_MSGPACK_SCHEMA);
		...
		if (!deriveSchema && hasSchema && hasSchemaString) {
			throw new ValidationException("A definition of both a schema and MSGPACK schema is not allowed.");
		} else if (!deriveSchema && !hasSchema && !hasSchemaString) {
			throw new ValidationException("A definition of a schema or MSGPACK schema is required " +
				"if derivation from table's schema is disabled.");
		} else if (hasSchema) {
			properties.validateType(FORMAT_SCHEMA, false, true);
		} else if (hasSchemaString) {
			properties.validateString(FORMAT_MSGPACK_SCHEMA, false, 1);
		}
